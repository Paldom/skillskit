...
discover the same five skills via the repo's `.agents/skills/` — no install
needed.
...
| [upgrade-repo](skills/upgrade-repo/) | Carries later skillskit improvements back into a repo scaffolded by an older version — refreshes the validator, eval runner, hooks and docs, and reports the customized files to merge by hand. |
