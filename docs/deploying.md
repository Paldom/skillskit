...
- Consumers run `npx skills add Paldom/skillskit` — the CLI
  ...
  users can also `gh skill install Paldom/skillskit <skill>
...
gh repo edit Paldom/skillskit --add-topic skills-sh
#    (gh < 2.90 has no `gh skill`; equivalent fallback:
#     gh release create v0.1.0 --generate-notes
#     gh repo edit Paldom/skillskit --add-topic agent-skills --add-topic skills-sh)
...
npx skills@latest add Paldom/skillskit --list
mkdir -p /tmp/skills-verify && cd /tmp/skills-verify
env -u DISABLE_TELEMETRY -u DO_NOT_TRACK npx skills@latest add Paldom/skillskit -a claude-code -y   # NO --skill '*': the wildcard opts into internal dev skills
...
curl -sIL https://skills.sh/Paldom/skillskit | head -1
```
