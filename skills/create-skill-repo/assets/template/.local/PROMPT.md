...
2. Research beyond `.local/`: web-search current facts, official docs, prior art; verify every load-bearing claim against primary sources. Verified findings go into each skill's references/ (never cite `.local/` paths).
...
8. After each skill: update the README catalog, CHANGELOG.md, and its skills.sh.json grouping (exactly one group each; engaging one-line group descriptions - that file is the skills.sh listing copy).
9. If the skills compose into a workflow, author docs/setup-prompt.md: a paste-ready /goal (max 4000 chars) orchestrating them - ordering, parallel agents only on disjoint files, per-skill verifier gates, no git actions. Fact-check its commands against the shipped skills; link it from the README.
10. README LAST: read docs/readme-standard.md and follow it. Icon via /icon-brief -> /icon-draw -> /icon-critique (show me candidates; the pick is mine). Demo committed with its source (/tape-demo, or /walkthrough-storyboard + /walkthrough-record for a browser). Then /deslop on README PROSE ONLY - never code, commands, flags, paths, numbers, headings, table cells or step markers, and never soften a claim, negation or limitation. Re-run `make check` after it.
...
- README passes docs/readme-standard.md (icon, demo + source, skills-level examples only, marked steps, deslopped prose)
- a triggering self-test (description-only routing vs the full catalog) per skill, reported in your final summary
